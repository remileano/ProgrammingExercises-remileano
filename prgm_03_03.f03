...
!	file, and loads a packed form of a symmetric matrix. Then,
!	the packed matrix is expanded assuming a column-wise lower-triangle form
!	and printed. Finally, the matrix is diagonalized using the LAPack routine
!	SSPEV. The eigenvalues and eigenvectors are printed.
!
!	The input file is expected to have the leading dimension (an integer
!	NDim) of the matrix on the first line. The next (NDim*(NDim+1))/2 lines each
...
!
	Write(*,*)' The matrix loaded (column-wise) lower-triangle packed:'
...
!
!	This subroutine accepts an array, ArrayIn, that is (N*(N+1))/2 long. It then
!	converts that form to the  N-by-N matrix AMatOut taking ArrayIn to be in lower-
!	packed storage form. Note: The storage mode also assumes the lower-packed storage
!	is packed by columns.
!
	Implicit None
	Integer,Intent(In)::N
...
!
!	This subroutine accepts an array, ArrayIn, that is (N*(N+1))/2 long. It then
!	converts that form to the N-by-N matrix AMatOut taking ArrayIn to be in
!	upper-packed storage form. Note: The storage mode also assumes the upper-packed
!	storage is packed by columns.
